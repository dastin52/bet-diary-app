...
const LoadingSkeleton: React.FC = () => (
    <div className="space-y-3">
        {[...Array(3)].map((_, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg animate-pulse">
            ...
            <div className="space-y-2">
                {matches.map((match, index) => (
                    <button 
                        ...
                        className="w-full text-left p-3 rounded-lg hover:bg-gray-700/80 transition-colors duration-200 flex justify-between items-center"
                    >
                        <div>
                            <p className="text-xs text-gray-400">{match.sport} &middot; {match.eventName}</p>
                            <p className="font-semibold text-white flex items-center gap-2">
                                {match.isHotMatch && <FireIcon />}
                                {match.teams}
                            </p>
                            <p className="text-sm text-indigo-300">{match.date} &middot; {match.time}</p>
                        </div>
