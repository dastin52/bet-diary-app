...
export const useAdminData = (): UseAdminDataReturn => {
  ...
  const analytics = useMemo((): AdminAnalytics | null => {
    ...
    const popularSportsCounts = settledBets.reduce((acc: Record<string, number>, bet) => {
        ...
        return acc;
    // FIX: Cast initial value of reduce to Record<string, number> to ensure correct type inference for 'popularSportsCounts'.
    ...
    const popularBookmakersCounts = settledBets.reduce((acc: Record<string, number>, bet) => {
        ...
        return acc;
    // FIX: Cast initial value of reduce to Record<string, number> to ensure correct type inference for 'popularBookmakersCounts'.
    }, {} as Record<string, number>);
